...
const modules = [
  { name: 'Ruang Pulih',      sub: 'Ruang Aman untuk Belajar',          bg: '#3A8FA3    ', href: '#ruang-pulih',      icon: 'uil-heart-sign' },
  { name: 'Seni Merasa',      sub: 'Belajar Memahami Perasaan',          bg: '#D45B5B', href: '#seni-merasa',      icon: 'uil-palette' },
  { name: 'Sains & Karakter', sub: 'Belajar Cara Otak Bekerja',          bg: '#2D6FA4', href: '#sains-karakter',   icon: 'uil-brain' },
  { name: 'Nutrisi Kognitif', sub: 'Makanan untuk Otak yang Sehat',      bg: '#D4690A', href: '#nutrisi-kognitif', icon: 'uil-heart-medical' },
  { name: 'Napas Logika',      sub: 'Belajar Berpikir Tenang dan Jelas', bg: '#7B68C8', href: '#napas-logika',      icon: 'uil-wind' },
  { name: 'Liputan Media',    sub: 'Cerita dan Dokumentasi Kegiatan',   bg: '#2E8B57', href: '#liputan-media',    icon: 'uil-megaphone' },
];

// Section colour palettes
const PALETTES = {
  'ruang-pulih':      { left: '#245966', right: '#3A8FA3    ', accent: '#A1D6E2', textCta: '#ffffff' },
  'sains-karakter':   { left: '#1D476A', right: '#2D6FA4', accent: '#90CAF9', textCta: '#ffffff' },
  'napas-logika':     { left: '#4E4282', right: '#7B68C8', accent: '#CE93D8', textCta: '#ffffff' },
  'seni-merasa':      { left: '#873A3A', right: '#D45B5B', accent: '#EF9A9A', textCta: '#ffffff' },
  'nutrisi-kognitif': { left: '#874306', right: '#D4690A', accent: '#FFCC80', textCta: '#ffffff' },
  'liputan-media':    { left: '#1E5938', right: '#2E8B57', accent: '#A5D6A7', textCta: '#ffffff' },
};
...
const napasArticles = [
  {
    ...
    excerpt: 'Di tengah kemudahan yang ditawarkan AI, manusia perlahan kehilangan kebiasaan berpikir kritis dan reflektif. Bukan karena teknologi salah, tetapi karena kita mulai berhenti mempertanyakan dan terlalu cepat menerima jawaban yang terlihat rapi.',
    pdfUrl: '/pdfs/Napas-Logika-1.pdf',
  },
  {
    author: "Mohammad Imam Ma'ruf, M.Sc.",
    title: 'Menyalakan Api, Bukan Sekadar Mengisi Presensi: Menggugat Panggilan Sejati Seorang Dosen',
    excerpt: 'Di tengah pembangunan fisik yang terus meningkat, kita sering melupakan peran dosen sebagai perancang cara berpikir generasi yang akan mengelolanya. Lebih dari sekadar pengajar, dosen adalah penjaga akal sehat, pembentuk karakter, dan penggerak perubahan yang menjaga agar ilmu tetap manusiawi dan berdampak nyata.',
...
const liputanArticles = [
  ...
  },
  {
    ...
    excerpt: 'Diskusi tentang konsep belajar relasional ini disampaikan dalam dialog di Studio Pro 2 RRI Semarang pada Senin pagi, 30 Maret 2026. Percakapan membahas bagaimana rasa aman, relasi emosional, dan kehadiran pendidik menjadi fondasi penting dalam pendidikan modern, terutama di tengah meningkatnya kecemasan pada anak dan remaja.',
...
function SectionShell({ id, tag, title, quote, icon, leftStyle = {}, rightStyle = {}, minH = 580, p, children }) {
  ...
  return (
    <section id={id} className="relative overflow-hidden block m-0 p-0">
      <div className={`flex flex-col lg:flex-row h-full lg:min-h-[580px]`}>
        ...
        >
          {icon && (
            <i className={`uil ${icon} absolute -top-10 -left-12 text-[15rem] lg:text-[20rem] text-white/[0.04] pointer-events-none select-none`} />
          )}
          ...
          <h2
            ...
            style={{ fontSize: 'clamp(3.2rem, 6.5vw, 5.5rem)', lineHeight: 0.93 }}
          >
            {title}
          </h2>
          {quote && (
        ...
        {/* RIGHT — Content */}
        <div className="lg:w-7/12 flex flex-col justify-center p-10 lg:p-16 gap-8 relative z-10" style={rightStyle}>
          {children}
        </div>
      </div>
    </section>
  ...
  return (
    ...
    >
      ...
      >
        {/* Header bar */}
        <div 
          className="flex items-center justify-between px-5 py-3.5 shrink-0 bg-[#1a1a1a] border-b border-white/10"
        >
...
function ArticlesList({ articles, p, ctaText, onOpenPdf }) {
  return (
    <>
      {articles.map((a, i) => {
        return (
          <div key={i} className={i < articles.length - 1 ? 'pb-8' : ''} style={{ borderBottom: i < articles.length - 1 ? `1px solid rgba(255,255,255,0.4)` : 'none' }}>
            {a.meta   && <p className="font-inter text-xs italic mt-4 mb-1 text-white/35">{a.meta}</p>}
            <h3 className="font-rubik font-semibold text-xl text-white leading-tight mb-3">{a.title}</h3>
            {a.author && <p className="font-inter text-xs font-medium uppercase tracking-[0.12em] text-white mb-4 bg-black/10 px-2 py-1 w-fit">Penulis: {a.author}</p>}
            <p className="font-inter text-sm text-white/80 leading-relaxed mb-4">{a.excerpt}</p>
            {a.link ? (
              <a href={a.link} target="_blank" rel="noopener noreferrer" className="hover:underline font-inter text-sm font-medium inline-flex items-center gap-1 hover:opacity-70 transition-opacity cursor-pointer bg-transparent border-0 p-0"
              style={{ color: p.textCta }}
            >{ctaText || 'Baca selengkapnya, klik di sini.'} <i className="uil uil-arrow-right text-base" /></a>
            ) : a.pdfUrl ? (
              <button
                ...
                style={{ color: p.textCta }}
              >
                {ctaText || 'Baca selengkapnya, klik di sini.'} <i className="uil uil-arrow-right text-base" />
              </button>
            ) : (
              <span
...
export default function ArticlesPage() {
  ...
  useEffect(() => {
    const handleScroll = () => setVisible(document.documentElement.scrollTop > 300);
  ...
  return (
    <>
      <div className="flex flex-col min-h-screen md:mt-10">
        ...
        {/* ─── 1. HERO ─── */}
        <section id="home" className="flex flex-col lg:flex-row w-full m-0 p-0 overflow-hidden bg-amber-500 lg:min-h-screen">

          {/* Left: brand panel dengan full background image */}
          <div className="lg:w-1/2 relative flex flex-col overflow-hidden" style={{ minHeight: '50vh' }}> 
            {/* Background Image */}
            <img 
              src={TeamImage} 
            ...
            <div className="relative z-20 pt-80 lg:pt-32 px-10 lg:px-14 pb-10">
              ...
              <p className="text-white/85 font-inter font-semibold uppercase mt-3" style={{ fontSize: 'clamp(0.55rem, 1.2vw, 0.75rem)', letterSpacing: '0.22em' }}>
          ...
          {/* Right: module grid */}
          <div className="lg:w-1/2 grid grid-cols-2" style={{ gridTemplateRows: 'repeat(3, 1fr)' }}>
            {modules.map((mod, i) => (
              <button
                key={i}
              ...
              >
                ...
                {mod.icon && (
                  <i className={`uil ${mod.icon} absolute -bottom-6 -right-6 text-[7rem] lg:text-[10rem] text-white/[0.04] pointer-events-none select-none`} />
                )}

                <span className="text-white leading-tight mb-2 relative z-10" style={{ fontFamily: 'var(--font-dancing)', fontSize: 'clamp(1.4rem, 2.8vw, 2.1rem)', fontWeight: 700}}>
                  {mod.name}
                </span>
                <span className="text-white/85 font-inter text-xs lg:text-sm leading-snug relative z-10">{mod.sub}</span>
              </button>
            ))}
          </div>
        </section>
        ...
        <SectionShell
          ...
          p={rp}
        >
          <ArticlesList onOpenPdf={handleOpenPdf} articles={ruangPulihArticles} p={rp} />
        ...
        <SectionShell
          ...
          p={sk}
        >
          <ArticlesList onOpenPdf={handleOpenPdf} articles={sainsArticles} p={sk} />
          {/* <div className="flex justify-end"><ArrowBtn /></div> */}
        </SectionShell>
        ...
        <SectionShell
          ...
          p={nl}
        >
          <ArticlesList onOpenPdf={handleOpenPdf} articles={napasArticles} p={nl} />
          {/* <div className="flex justify-end"><ArrowBtn /></div> */}
        </SectionShell>
        ...
        <section id="liputan-media" className="relative overflow-hidden block m-0 p-0" style={{ background: lm.left }}>
          ...
          <div className="flex flex-col lg:flex-row" style={{ minHeight: 580 }}>
            ...
            <div className="lg:w-7/12 flex flex-col justify-center p-10 lg:p-16 gap-8 relative overflow-hidden">
              ...
              <div className="relative z-10">
                <h2 className="font-rubik font-black text-white leading-none" style={{ fontSize: 'clamp(3.2rem, 6.5vw, 5.5rem)', lineHeight: 0.93 }}>
                  LIPUTAN<br />MEDIA
                </h2>
                <div style={{ width: 36, height: 3, background: lm.accent, borderRadius: 2, marginTop: 16 }} />
              </div>
              <div className="relative z-10">
                <ArticlesList onOpenPdf={handleOpenPdf} articles={liputanArticles} p={lm} ctaText="Baca selengkapnya, klik di sini." />
              </div>
            ...
            <div
              ...
              style={{ backgroundImage: `url(${LiputanMediaImage})`, backgroundSize: 'cover', backgroundPosition: 'center' }}
            >
              <div className="absolute inset-0" style={{ background: 'rgba(30,89,56,0.0)' }} />
            </div>
          </div>
        </section>
